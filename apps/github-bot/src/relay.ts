import type { RelayRequest, StreamChunk } from '@agent-relay/core'
...
/**
 * Run relay with the default provider (from RELAY_PROVIDER env).
 * Consumes runStream and collects text; on error returns user-facing message without internal details.
 */
export async function runRelay(prompt: string, workspace: string): Promise<RelayOutcome> {
